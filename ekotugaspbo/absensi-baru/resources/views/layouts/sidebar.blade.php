@php
    ...
    // Helper function untuk cek status active agar code lebih bersih
    function isActive($path) {
        return request()->is($path) ? 'active text-white bg-primary' : '';
    }
@endphp
...
<div class="card border-0 shadow-sm p-3" style="border-radius: 15px;">
    ...
    <div class="list-group list-group-flush">
        ...
        {{-- KELOLA MENU (Hanya jika diizinkan di DB) --}}
        @if(in_array('kelola-menu', $userPermissions))
            <div class="mt-3 mb-1 small fw-bold text-muted px-2">KELOLA MENU</div>
            <a href="{{ route('access.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/access-menu*') }}">
        ...
        @if(in_array('kelola-absensi', $userPermissions))
            ...
            <a href="{{ route('libur.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/libur*') }}">
                <i class="bi bi-calendar-x me-2"></i> Atur Libur
            </a>
            <a href="{{ route('absensi.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/absensi*') }}">
                <i class="bi bi-clipboard-data me-2"></i> Data Absensi
            </a>
            <a href="{{ route('rekap.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/rekap*') }}">
                <i class="bi bi-file-earmark-bar-graph me-2"></i> Rekap Absen
            </a>
            <a href="{{ route('jam.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/jam*') }}">
        ...
        @if(in_array('siswa', $userPermissions))
            ...
            <a href="{{ route('siswa.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/siswa') }}">
                <i class="bi bi-people me-2"></i> Data Seluruh Siswa
            </a>
            <a href="{{ route('kelas.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/kelas*') }}">
                <i class="bi bi-building me-2"></i> Kelas & Jurusan
            </a>
            <a href="{{ route('siswa.perkelas') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/siswa-per-kelas*') }}">
                <i class="bi bi-mortarboard me-2"></i> Siswa per kelas
            </a>
            <a href="{{ route('izin.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/izin*') }}">
        ...
        @if(in_array('users', $userPermissions))
            ...
            <a href="{{ route('users.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/users*') }}">
        ...
        @if(in_array('api-config', $userPermissions))
            ...
            <a href="{{ route('admin.api.index') }}" class="list-group-item list-group-item-action border-0 rounded-3 mb-1 {{ isActive('admin/menu/api*') }}">
        ...
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="list-group-item list-group-item-action border-0 rounded-3 text-danger">
